// SPDX-License-Identifier: BSD-3-Clause
// AUTOGENERATED by tools/codegen/emit_java.py - do not edit.
package whiteout.m3;

import java.lang.foreign.*;
import java.lang.invoke.MethodHandle;
import java.nio.charset.StandardCharsets;
import whiteout.common.*;
import whiteout.common.internal.Handles;
import whiteout.common.internal.NativeCommon;
import whiteout.m3.internal.Native;

/**
 * One decoded property of a data-driven material
 * 
 * `data` is the raw value; its shape depends on `size`: 4 = scalar (f32, u32 enum, or BGRA colour — depends on the property), 8 = `{u32 index into DataDrivenMaterial::texturePaths, u32 texture source}`, 12 = 3 floats, 16 = 4 floats, 20 = `{u32 ColorChannelSelect, f32 multiply, f32 add, f32, u16 flags}`, 32 = `{f32 offsetU/V, tilingU/V, angleU/V/W, u32 flags}`, 48 = fresnel `{u32 FresnelMode, f32 exponent, min, max, rotation, mask}`.
 *
 * <p><b>Lifecycle.</b> Instances hold a handle to a native
 * DataDrivenProperty allocation. Always release them with
 * {@link #close()} — try-with-resources is the natural shape:
 * <pre>{@code
 * try (DataDrivenProperty obj = ...) {
 *     // ... use obj ...
 * }
 * }</pre>
 * Borrowed views returned by field getters (e.g. nested struct slices)
 * share the parent's memory and must not outlive it.
 *
 * <p><b>Thread-safety.</b> Instances are not thread-safe. The underlying
 * C++ types make no synchronization guarantees, so callers must coordinate
 * external access if a handle is shared across threads.
 */
public final class DataDrivenProperty implements AutoCloseable {
    private static final long BYTES = 72L;

    final MemorySegment handle;
    final boolean owned;

    DataDrivenProperty(MemorySegment seg, boolean owned) {
        this.handle = (seg == null || seg.equals(MemorySegment.NULL))
            ? seg : seg.reinterpret(BYTES);
        this.owned = owned;
    }

    public DataDrivenProperty() {
        MemorySegment __raw = (MemorySegment) NativeCommon.invokeNative(Native.whiteout_m3_M3DataDrivenProperty_new);
        if (__raw == null || __raw.equals(MemorySegment.NULL))
            throw new RuntimeException("DataDrivenProperty allocation failed");
        this.handle = __raw.reinterpret(BYTES);
        this.owned = true;
    }

    @Override
    public void close() {
        if (!owned) return;
        if (handle != null && !handle.equals(MemorySegment.NULL)) {
            NativeCommon.invokeNative(Native.whiteout_m3_M3DataDrivenProperty_delete, handle);
        }
    }

    /**
     * crc32 of the property name
     * @return the nameHash field of this M3DataDrivenProperty.
     */
    public int getNameHash() {
        return handle.get(ValueLayout.JAVA_INT, 0L);
    }
    public void setNameHash(int value) {
        handle.set(ValueLayout.JAVA_INT, 0L, value);
    }
    /**
     * Resolved name, empty when the hash is unknown
     * @return the name field of this M3DataDrivenProperty.
     */
    public String getName() {
        try (Arena arena = Arena.ofConfined()) {
            MemorySegment __s = (MemorySegment) NativeCommon.invokeNative(Native.whiteout_m3_M3DataDrivenProperty_get_name, arena, handle);
            MemorySegment __chars = __s.get(ValueLayout.ADDRESS, 0);
            long __len = __s.get(ValueLayout.JAVA_LONG, ValueLayout.ADDRESS.byteSize());
            if (__chars == null || __chars.equals(MemorySegment.NULL)) return "";
            byte[] __bytes = __chars.reinterpret(__len).toArray(ValueLayout.JAVA_BYTE);
            NativeCommon.invokeNative(Native.whiteout_CString_free, __s);
            return new String(__bytes, StandardCharsets.UTF_8);
        }
    }
    public void setName(String value) {
        try (Arena arena = Arena.ofConfined()) {
            byte[] __utf = value == null ? new byte[0] : value.getBytes(StandardCharsets.UTF_8);
            MemorySegment __seg = arena.allocate(__utf.length + 1);
            MemorySegment.copy(__utf, 0, __seg, ValueLayout.JAVA_BYTE, 0, __utf.length);
            __seg.set(ValueLayout.JAVA_BYTE, __utf.length, (byte) 0);
            NativeCommon.invokeNative(Native.whiteout_m3_M3DataDrivenProperty_set_name, handle, __seg);
        }
    }
    /**
     * Raw value bytes
     * @return the data field of this M3DataDrivenProperty.
     */
    public int getDataCount() {
        return (int) (long) NativeCommon.invokeNative(Native.whiteout_m3_M3DataDrivenProperty_get_data_count, handle);
    }
    public byte[] getData() {
        long __count = (long) NativeCommon.invokeNative(Native.whiteout_m3_M3DataDrivenProperty_get_data_count, handle);
        MemorySegment __ptr = (MemorySegment) NativeCommon.invokeNative(Native.whiteout_m3_M3DataDrivenProperty_get_data_data, handle);
        if (__count == 0 || __ptr == null || __ptr.equals(MemorySegment.NULL)) return new byte[0];
        long __scalars = __count * 1L;
        return __ptr.reinterpret(__scalars * 1L).toArray(ValueLayout.JAVA_BYTE);
    }
    public void setData(byte[] values) {
        try (Arena arena = Arena.ofConfined()) {
            long __count = (long) values.length / 1;
            MemorySegment __seg = arena.allocate((long) values.length * 1L);
            if (values.length > 0) MemorySegment.copy(values, 0, __seg, ValueLayout.JAVA_BYTE, 0, values.length);
            NativeCommon.invokeNative(Native.whiteout_m3_M3DataDrivenProperty_assign_data, handle, __seg, __count);
        }
    }
    public void resizeData(int count) {
        NativeCommon.invokeNative(Native.whiteout_m3_M3DataDrivenProperty_resize_data, handle, (long) count);
    }
    @Override public String toString() {
        return "DataDrivenProperty(" + "nameHash=" + getNameHash() + ", " + "name=" + getName() + ")";
    }

}
