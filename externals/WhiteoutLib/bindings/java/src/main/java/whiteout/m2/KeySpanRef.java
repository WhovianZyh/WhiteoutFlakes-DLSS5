// SPDX-License-Identifier: BSD-3-Clause
// AUTOGENERATED by tools/codegen/emit_java.py - do not edit.
package whiteout.m2;

import java.lang.foreign.*;
import java.lang.invoke.MethodHandle;
import java.nio.charset.StandardCharsets;
import whiteout.common.*;
import whiteout.common.internal.Handles;
import whiteout.common.internal.NativeCommon;
import whiteout.m2.internal.Native;

/**
 * One sequence's slice of a key array, as the file writes it: a count and an offset into whichever file holds that sequence's keys.
 * 
 * Kept only by a lazily parsed model (Parser::setLazyAnimations), which leaves the sub-arrays of externally-stored sequences empty until loadSequence() reads their `.anim` sibling. Nothing else needs it: an eager parse consumes the reference on the spot.
 *
 * <p><b>Lifecycle.</b> Instances hold a handle to a native
 * KeySpanRef allocation. Always release them with
 * {@link #close()} — try-with-resources is the natural shape:
 * <pre>{@code
 * try (KeySpanRef obj = ...) {
 *     // ... use obj ...
 * }
 * }</pre>
 * Borrowed views returned by field getters (e.g. nested struct slices)
 * share the parent's memory and must not outlive it.
 *
 * <p><b>Thread-safety.</b> Instances are not thread-safe. The underlying
 * C++ types make no synchronization guarantees, so callers must coordinate
 * external access if a handle is shared across threads.
 */
public final class KeySpanRef implements AutoCloseable {
    private static final long BYTES = 8L;

    final MemorySegment handle;
    final boolean owned;

    KeySpanRef(MemorySegment seg, boolean owned) {
        this.handle = (seg == null || seg.equals(MemorySegment.NULL))
            ? seg : seg.reinterpret(BYTES);
        this.owned = owned;
    }

    public KeySpanRef() {
        MemorySegment __raw = (MemorySegment) NativeCommon.invokeNative(Native.whiteout_m2_M2KeySpanRef_new);
        if (__raw == null || __raw.equals(MemorySegment.NULL))
            throw new RuntimeException("KeySpanRef allocation failed");
        this.handle = __raw.reinterpret(BYTES);
        this.owned = true;
    }

    @Override
    public void close() {
        if (!owned) return;
        if (handle != null && !handle.equals(MemorySegment.NULL)) {
            NativeCommon.invokeNative(Native.whiteout_m2_M2KeySpanRef_delete, handle);
        }
    }

    /** @return the count field of this M2KeySpanRef. */
    public int getCount() {
        return handle.get(ValueLayout.JAVA_INT, 0L);
    }
    public void setCount(int value) {
        handle.set(ValueLayout.JAVA_INT, 0L, value);
    }
    /** @return the offset field of this M2KeySpanRef. */
    public int getOffset() {
        return handle.get(ValueLayout.JAVA_INT, 4L);
    }
    public void setOffset(int value) {
        handle.set(ValueLayout.JAVA_INT, 4L, value);
    }
    @Override public String toString() {
        return "KeySpanRef(" + "count=" + getCount() + ", " + "offset=" + getOffset() + ")";
    }

}
